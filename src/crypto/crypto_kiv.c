...
int encryptKIV(SymKey_t* key, SymIV_t* iv, PubKey_t pubKey, uint8_t** encBytes) {
	...
	RSA_public_encrypt(KIV_PAIR_LENGTH, (unsigned char*) kivPair, (unsigned char*) (*encBytes), (RSA*) pubKey, KIV_PADDING);
...
int decryptKIV(uint8_t* encBytes, SecKey_t secKey, SymKey_t* key, SymIV_t* iv) {
	...
	RSA_private_decrypt(DHTD_ENC_KIVPAIR_BYTE_LENGTH, (unsigned char*) encBytes, (unsigned char*) kivPair, (RSA*) secKey, KIV_PADDING);
