...
int encryptKIV(SymKey_t* key, SymIV_t* iv, PubKey_t pubKey, uint8_t** encBytes) {
	if( key == NULL )      { ERRORMSG("encryptKIV: key is NULL\r\n");      return DHTD_ERROR; }
	if( iv == NULL )       { ERRORMSG("encryptKIV: iv is NULL\r\n");       return DHTD_ERROR; }
	if( pubKey == NULL )   { ERRORMSG("encryptKIV: pubKey is NULL\r\n");   return DHTD_ERROR; }
	if( encBytes == NULL ) { ERRORMSG("encryptKIV: encBytes is NULL\r\n"); return DHTD_ERROR; }
	
	int err = 0;
	
	(*encBytes) = malloc(sizeof(uint8_t)*DHTD_ENC_KIVPAIR_BYTE_LENGTH);
	if( (*encBytes) == NULL ) { ERRORMSG("encryptKIV: NULL pointer allocated to encBytes\r\n"); return DHTD_ERROR; }
	...
	if( err == -1 ) {
		...
		ERRORMSG("encryptKIV: Error in RSA_public_encrypt (%i: %s)\r\n",rsaErr,ERR_error_string(rsaErr,NULL));
		return DHTD_ERROR;
		}
...
int decryptKIV(uint8_t* encBytes, SecKey_t secKey, SymKey_t* key, SymIV_t* iv) {
	if( encBytes == NULL ) { ERRORMSG("decryptKIV: encBytes is NULL\r\n"); return DHTD_ERROR; }
	if( secKey == NULL )   { ERRORMSG("decryptKIV: secKey is NULL\r\n");   return DHTD_ERROR; }
	if( key == NULL )      { ERRORMSG("decryptKIV: key is NULL\r\n");      return DHTD_ERROR; }
	if( iv == NULL )       { ERRORMSG("decryptKIV: iv is NULL\r\n");       return DHTD_ERROR; }
	...
	if( err == -1 ) {
		int rsaErr = ERR_peek_last_error();
		ERRORMSG("decryptKIV: Error in RSA_private_decrypt (%i: %s)\r\n",rsaErr,ERR_error_string(rsaErr,NULL));
		return DHTD_ERROR;
		}
